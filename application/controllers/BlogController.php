...
//          if (!$conduitXml = $this->tryCache($cacheName, $cacheLife)) {
            // cache miss
            $query      = new Zend_Gdata_Query('http://www.blogger.com/feeds/' . Zend_Registry::get('config')->blogs->blogger->conduitBlogId . '/posts/default/' . $input->entryId);
            $entry      = $gdClient->getFeed($query);
            $entryXml   = $entry->getXML();
//            $conduitXml = $this->saveCache($conduitXml, $cacheName, $cacheLife);
            Zend_Debug::dump($entryXml);
        }

        exit();

        $blogXml  = '<blogs>';
        $blogXml .= '<conduit>' . $conduitXml . '</conduit>';
        $blogXml .= '<divert>' . $divertXml . '</divert>';
        $blogXml .= '</blogs>';

        $this->view->xml .= $blogXml;
    }
}
